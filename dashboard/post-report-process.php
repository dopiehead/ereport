...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Validate the first file upload
    ...
    // Validate the second file upload
    ...
    if ($uploadSuccess) {
        ...
        $stmt = $conn->prepare("INSERT INTO report (user_id, reporterName, addressOffender, eventDate, eventTime, eventDetails, reportPurpose, anonymous, reportTo, reportCategory, fileupload, images, date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        ...
        $stmt->bind_param("issssssssssss", $user_id, $reporterName, $addressOffender, $eventDate, $eventTime, $eventDetails, $reportPurpose, $anonymous, $selectedreportTo, $selectedreportCategory, $myimage, $myimagex, $date);
        if ($stmt->execute()) {
